#!/usr/bin/env python3
"""
Append-only BTC checkpoint helper.

This creates a small frozen checkpoint folder under research/btc/checkpoints/<id>/
and appends a matching entry to research/btc/checkpoints/index.json.
"""
...
RESEARCH_ROOT = Path("research/btc")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Freeze a BTC research checkpoint into the bookkeeping ledger.")
    parser.add_argument("--checkpoint-id", required=True, help="Stable checkpoint id, e.g. btc-meanrev-catboost-30s-v1")
    parser.add_argument("--status", required=True, help="Status label, e.g. champion, challenger_failed, collecting")
    parser.add_argument("--category", required=True, help="Category label, e.g. continuation, mean_reversion, lob")
    parser.add_argument("--summary", required=True, help="One-line checkpoint summary")
    parser.add_argument("--artifact", action="append", default=[], help="Artifact as key=absolute_or_repo_path")
    parser.add_argument("--metric", action="append", default=[], help="Metric as key=value")
    parser.add_argument("--note-file", help="Optional note markdown/txt file to copy into the checkpoint folder")
